...
class Martfury_WooCommerce {
	...
	function __construct() {
		...
		add_action( 'woocommerce_single_product_summary', array( $this, 'template_single_summary_stock' ), 19 );
	...
	*/
	...
	function template_single_summary_header() {
		...
		if ( in_array( $product->get_type(), array( 'simple', 'variable' ) ) ) {
			...
			if($value>35){?>
				<p class="terms price-currency"><label>prijs in €</label></p>
							...
							<div class="steps-form">
								<div class="steps-row setup-panel">
									...
									<div class="steps-step">
										<p><?php echo str_replace(".",",",number_format(round(($value)/3,2),2)); ?></p>				    
										<a href="#step-9" class="btn-indigo btn-circle installmentOption"><i class="fa fa-circle"></i></a>
										<p>3 keer</p>
									</div>
									<?php } ?>
				<?php
				if($value>250 && $isSmartPhoneCat == false){
					$output[] = sprintf( '
									<div class="steps-step">
										<p>'.str_replace(".",",",number_format(round(($value)*0.08772,2),2)).'</p>
										<a href="#step-10" class="btn-default btn-circle installmentOption" disabled="disabled"><i class="fa fa-circle"></i></a>
										<p>12 keer</p>
									</div>
									<div class="steps-step">
										<p>'.str_replace(".",",",number_format(round(($value)*0.05984,2),2)).'</p>
										<a href="#step-11" type="button" class="btn-default btn-circle installmentOption" disabled="disabled"><i class="fa fa-circle"></i></a>
										<p>18 keer</p>
									</div>
									<div class="steps-step">
										<p>'.str_replace(".",",",number_format(round(($value)*0.04594,2),2)).'</p>
										<a href="#step-11" class="btn-default btn-circle installmentOption" disabled="disabled"><i class="fa fa-circle"></i></a>
										<p>24 keer</p>
									</div>
									<div class="steps-step">
										<p>'.str_replace(".",",",number_format(round(($value)*0.03206,2),2)).'</p>
										<a href="#step-11" class="btn-default btn-circle installmentOption" disabled="disabled"><i class="fa fa-circle"></i></a>
										<p>36 keer</p>
									</div>
									<!--<div class="steps-step">
										<p>'.get_woocommerce_currency_symbol()." ".str_replace(".",",",number_format(round(($value),2),2)).'</p>
										<a href="#step-11" class="btn-default btn-circle installmentOption" disabled="disabled"><i class="fa fa-circle"></i></a>
										<p>Single</p>
									</div>-->
								</div>
				...
				//<div class="mf-summary-meta">%s</div>', wc_get_stock_html( $product ) );
				if($value>600)
				{
					echo sprintf( '%s</div><p class="terms aantal-termijnen"><label>Aantal termijnen</label><span class="popular-price-term">(<span class="term"><bdi>Vaak betaald in 12 termijnen</bdi></span>)</span></p>', implode( ' ', $output ) );
				}else{
					echo sprintf( '%s</div><p class="terms aantal-termijnen"><label>Aantal termijnen</label><span class="popular-price-term">(<span class="term"><bdi>Vaak betaald in 3 termijnen</bdi></span>)</span></p>', implode( ' ', $output ) );
				}
			}			
		}
	}
	 ...
	 * Simple stock
	 */
	function template_single_summary_stock() {
		global $product;
		...
		echo sprintf( '<div class="mf-summary-header-stock">%s</div>', implode( ' ', $output ) );
	}
		...
		if ( $custom_badges ) {
			...
			if ( ! $product->is_in_stock() && in_array( 'outofstock', $badges ) ) {
				...
				if ( martfury_get_option( 'sale_type' ) == '2' ) {
					...
					if ( $percentage ) {
						$output[] = '<span class="onsale ribbon"><span class="onsep"></span><span class="per"> Aanbieding</span>' . '</span>';
					}
				}
