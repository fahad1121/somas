...
function action_function_name_9559(){
    ...
    if($value>35){
        ...
        echo '<p><b>'. get_woocommerce_currency_symbol();
        echo str_replace(".",",",number_format(round(($value)/3,2),2)).'</b></p>';
        echo 'in 3 keer';
        echo '</div>';
    }
    if($value>250 && $isSmartPhoneCat == false){
        ...
        echo '<p><b>'. get_woocommerce_currency_symbol();
        echo str_replace(".",",",number_format(round(($value)*0.03206,2),2)).'</b></p>';
        echo 'in 36 keer';
        echo '</div>';
    }
    echo '</div>';
...
/** Shakir-20221213: Remove Payin3 payment for Apple products **/
/** Shakir-20230126: As per leon whatsapp on 23-Jan
 ** These categories should only be paid with in3 through pay.nl
 ** Smartphone (575, 582)
 ** Smartwatch (591)
 ** Koptelefoon (152, 184, 182, 177, 179, 178, 181, 183 ,176 ,180)
 ** Gaming (143, 294, 222, 50782, 229, 233, 295, 306, 307, 308, 304, 303, 309, 310, 311, 312, 296, 313, 314, 297, 319, 317, 315, 318, 316, 320, 321, 298, 322, 323, 324, 325, 326, 328, 327, 329, 299, 335, 337, 330, 334, 336, 333, 331, 332, 338)
 ** So we have payin3 with mollie for the rest and payin3 via pay for these categories
 ** Leon-Email-20230327: Please block iphones and else smartphones of €1.000 and more from Mondu.
 **/
function somas_unset_gateway_by_category( $available_gateways ) {
    ...
    $unset = false;
    $category_ids = array( 575, 582 );
    $setPayNlinThree = false;
    $category_ids_PayNlinThree = array( 575, 582, 591, 152, 184, 182, 177, 179, 178, 181, 183 ,176 ,180, 143, 294, 222, 50782, 229, 233, 295, 306, 307, 308, 304, 303, 309, 310, 311, 312, 296, 313, 314, 297, 319, 317, 315, 318, 316, 320, 321, 298, 322, 323, 324, 325, 326, 328, 327, 329, 299, 335, 337, 330, 334, 336, 333, 331, 332, 338 );
    $unsetMondu = false;
            ...
            if ( in_array( $term->term_id, $category_ids ) ) {
                ...
                if(!empty($values['data'])){
                    if(!empty($values['data']->price)){
                        if($values['data']->price>1000){
                            $unsetMondu=true;
                        }
                    }
                }
            }
            if ( in_array( $term->term_id, $category_ids_PayNlinThree ) ) {
                $setPayNlinThree = true;
            }
        }
    }

    $unsetPayinThree = false;
    $feed_ids = array( 42563 ); //this is YMS id in merk property
    $merk_ids = array( 2462 ); //this is apple id in merk property
    /*
      foreach ( WC()->cart->get_cart_contents() as $key => $values ) {
          //$_woo_product = wc_get_product( $values['product_id'] );
          //$_woo_product->get_sku();
          $termsfeed = get_the_terms( $values['product_id'], 'feed' );
          if(isset($termsfeed))
          {
              foreach ( $termsfeed as $termfeed ) {
                  if ( in_array( $termfeed->term_id, $feed_ids ) ) {
                      $unsetPayinThree = true;
                      break;
                  }
              }
          }

          if($unsetPayinThree != true){
              $termsMerk = get_the_terms( $values['product_id'], 'pa_merk' );
              if(isset($termsMerk))
              {
                  foreach ( $termsMerk as $termMerk ) {
                      if ( in_array( $termMerk->term_id, $merk_ids ) ) {
                          $unsetPayinThree = true;
                          break;
                      }
                  }
  ...
  */
    if ( $unset == true )
        unset( $available_gateways['pay_gateway_spraypay'] );

    if ( $setPayNlinThree == true ){
        unset( $available_gateways['mollie_wc_gateway_in3'] ); //mollie In3
    } else {
        unset( $available_gateways['pay_gateway_capayablegespreid'] ); //paynl In3
    }
    ...
    if ( $unsetMondu == true ){
        unset( $available_gateways['mondu_installment'] );
        unset( $available_gateways['mondu_invoice'] );
        unset( $available_gateways['mondu_direct_debit'] );
    }
...
// function somas_change_payment_gateway_title( $title, $gateway_id ){
// 	if( 'splitit' === $gateway_id ) {
// 		$title = 'Betaal met je creditcard in 4, 6 of 8 termijnen';
// 	}
// 	return $title;
// }
// add_filter( 'woocommerce_gateway_title', 'somas_change_payment_gateway_title', 25, 2 );
...
function somashome_add_custom_section_below_product() {
    echo '<center><img width="218" height="22" src="https://www.somashome.nl/wp-content/uploads/2022/05/trustpilot4.5Latest.svg" class="attachment-full size-full" alt="" loading="lazy" style="margin-top:10px;"></center>';
}
...
function new_order_reply_to_admin_header( $header, $email_id, $order ) {
    if ( $email_id === 'new_order' ){
        ...
        $header .= 'Reply-to: ' . __('SomasHome') . ' <' . 'order@somashome.nl' . ">\r\n";
    }
    return $header;
}
/** Shakir-20220830: Change reply to email from customer email to admin in New Order **/
...
function display_custom_checkout_delivery_date_jquery_script() {
    ...
    $foundBoxSpringItem = false;
    $boxspringItemcategories = array('1-persoons','2-persoons');
    // Only on front-end and checkout page
    if( is_checkout() && ! is_wc_endpoint_url() ) :
        foreach ( WC()->cart->get_cart() as $cart_item ) {
            ...
            if ( has_term( $boxspringItemcategories, 'product_cat', $cart_item['product_id'] ) ) {
                $foundBoxSpringItem = true;
                //break;
            }
        ...
        <script>
            jQuery(function($){
                ...
                var datesForDisable = ["2022-12-26"]
                var datesForDisableForBoxspring = ["2022-12-23","2022-12-24","2022-12-25","2022-12-26","2022-12-27","2022-12-28","2022-12-29","2022-12-30","2022-12-31","2023-01-01","2023-01-02","2023-01-03"]
                $(d).datepicker({
                    beforeShowDay: function(date) {
                        if (datesForDisable.length > 0) {
                            let currdateinprocess = date.getFullYear()+'-'+("0"+(date.getMonth()+1)).slice(-2)+'-'+("0"+date.getDate()).slice(-2);
                            for (var i = 0; i < datesForDisable.length; i++) {
                                if (currdateinprocess==datesForDisable[i]){
                                    return false;
                                }
                            }
                        }
                        if(<?php if($foundBoxSpringItem==true){ echo "true"; } else { echo "false"; } ?>){
                            if (datesForDisableForBoxspring.length > 0) {
                                let currdateinprocess = date.getFullYear()+'-'+("0"+(date.getMonth()+1)).slice(-2)+'-'+("0"+date.getDate()).slice(-2);
                                for (var i = 0; i < datesForDisableForBoxspring.length; i++) {
                                    if (currdateinprocess==datesForDisableForBoxspring[i]){
                                        return false;
                                    }
                                }
                            }
            ...
            jQuery("input[id='datepicker']").on('change',function(e){
                $isSaturday = false;
                if(jQuery('#datepicker').val()!="")
                {
                    var today = new Date(jQuery('#datepicker').datepicker('getDate'));
                    if (today.getDay() == 6) {
                        $isSaturday = true;
                    }
                }
...
{
    if (isset($_POST['isSaturday'])) {
        $isSaturday = sanitize_key($_POST['isSaturday']);
        WC()->session->set('isSaturday', $isSaturday);
        echo json_encode($isSaturday);
    }
    die(); // Alway at the end (to avoid server error 500)
}

// Add/remove delivery cost
add_action('woocommerce_cart_calculate_fees', 'add_delivery_cost', 20, 1);
function add_delivery_cost($cart)
{
    if (is_admin() && !defined('DOING_AJAX'))
        return;

    if( WC()->session->get('isSaturday') ){
        $cart->add_fee(__('Delivey cost', 'woocommerce'), 19.99);
    }else{
        $isFeePresent = false;
        $fees = WC()->cart->get_fees();
        foreach ($fees as $key => $fee) {
            if($fees[$key]->name === __('Delivey cost', 'woocommerce')) {
                unset($fees[$key]);
                $isFeePresent = true;
            }
        }
...
//     }
// 	if ($isSaturday == true) {
// 		WC()->cart->add_fee('Saturday Delivey cost', 19.99);
// 	}
// });
...
/** Shakir-20221207: WooCommerce Subscription Length Extender **/
...
/** Shakir-20230104: Change Catalog Ordering to product margin field **/
add_filter( 'woocommerce_get_catalog_ordering_args', 'enable_catalog_ordering_by_margin' );
function enable_catalog_ordering_by_margin( $args ) {

    if ( isset( $_GET['orderby'] ) ) {
        if ( 'margin' == $_GET['orderby'] ) {
            return array(
                'orderby'  => 'meta_value_num',
                'order'    => 'DESC',
                'meta_key' => 'product_custom_text_field_margin',
            );
        }
    }
...
add_filter( 'woocommerce_catalog_orderby', 'add_catalog_orderby_by_margin' );
function add_catalog_orderby_by_margin( $orderby_options ) {
    return array(
            'margin'	=> __("Standaard sortering", "woocommerce"),
        ) + $orderby_options ;
}

add_filter( 'woocommerce_default_catalog_orderby', 'default_catalog_orderby_margin' );
function default_catalog_orderby_margin( $default_orderby ) {
    return 'margin';
}

add_action( 'woocommerce_product_query', 'product_query_by_margin' );
function product_query_by_margin( $q ) {
    if ( ! isset( $_GET['orderby'] ) && ! is_admin() ) {
        $q->set( 'orderby', 'meta_value_num' );
        $q->set( 'order', 'DESC' );
        $q->set( 'meta_key', 'product_custom_text_field_margin');
    }
}
